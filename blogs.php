...
function blog_comments($blog_id, $fd) {
	$blog_cmt_qry = "SELECT m.members_name, bec.blog_entries_comments_blog_entries_id, bec.blog_entries_comments_date, bec.blog_entries_comments_comment " . " FROM members m, blog_entries_comments bec " . " WHERE bec.blog_entries_comments_blog_entries_id=$blog_id " . " AND m.members_id = bec.blog_entries_comments_author_id";
...
fputcsv($fd_blogcsv, split(',', "blog_id,author,title,date,content"));
fputcsv($fd_blogcmtscsv, split(',', "blog_id,comment_author,comment_date,comment_content"));
...
while (($blog_entry = mysql_fetch_array($qry_result, MYSQL_ASSOC))) {
	...
	fputcsv($fd_blogcsv, array($blog_entry['blog_entries_id'], $blog_entry['members_name'], $blog_entry['blog_entries_title'], 
		$blog_entry['blog_entries_last_updated_on'], $blog_entry['blog_entries_content']));
	...
	while (($cmt_entry = mysql_fetch_array($cmt_result, MYSQL_ASSOC)))  {
		...
		fputcsv($fd_blogcmtscsv, array($cmt_entry['blog_entries_comments_blog_entries_id'],$cmt_entry['members_name'], $cmt_entry['blog_entries_comments_date'], $cmt_entry['blog_entries_comments_comment']));
	}
}
?>
